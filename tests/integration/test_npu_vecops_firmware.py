"""Integration test for NPU vecops firmware: runs npu_vecops.elf and verifies output."""
...
class TestNpuVecopsFirmware:
    """Integration tests for firmware/npu_vecops."""

    def test_all_pass(self) -> None:
        """npu_vecops.elf prints ALL PASS and exits with code 0."""
        elf_path = FIRMWARE_DIR / "npu_vecops" / "npu_vecops.elf"
        if not elf_path.exists():
            pytest.skip("npu_vecops.elf not built (run: cd firmware/npu_vecops && make)")

        output, exit_code = _run_elf(elf_path)
        text = output.decode("utf-8")

        assert exit_code == 0, f"npu_vecops.elf exited with code {exit_code}"
    ...
    def test_individual_results(self) -> None:
        ...
        assert len(test_lines) >= 15, (
            f"Expected at least 15 test results, got {len(test_lines)}"
        )
